...
if (typeof _lan === "undefined") {
	if (window.location.href.indexOf("_lan") === -1) {
		var _lan = 2;
	} else {
		_lan = window.location.href.split("=")[1];
	}
};
...
HOME.Respond = function() {
	api = {
		...
		windowInit = function() {
			...
			$("#js-banner").css({
				"width": sizeArr[0] < newArr[0] ? newArr[0] : sizeArr[0] + "px",
				"height": sizeArr[1] + "px"
			});
			$("#js-narumi").css({
				"width": _width + "px",
				"height": _height + "px",
				"left": (_left - _width) / 2 + "px",
				"top": (sizeArr[1] - _height) / 2 + "px"
			});
			$("#js-copyright").css({
				"left": (_left - _width) / 2 + 20 + "px"
			});
			if ((newArr[0] < 761) && (newArr[0] > 380)) {
				$("#js-inner_inner").css({
					"left": (newArr[0] - 380) / 2 + 8 + "px",
					"width": "380px"
				});
			} else {
				if (newArr[0] <= 380) {
					$("#js-inner_inner").css({
						"left": (_left - _width) / 2 + 8 + "px",
						"width": newArr[0] + "px"
					});
				} else {
					$("#js-inner_inner").css({
						"left": _innerL + "px",
						"width": (newArr[0] - _innerL) + "px"
					});
					$("#js-txt_wrap").css({
						"width": (newArr[0] - _innerL) * .5 + "px"
					});
				}
			}
		}
...
HOME.BtnEff = function() {
	api = {
		...
		btnFadeOut = function(_id, _i) {
			$(_id).eq(_i).find("img").eq(1).stop().animate({
				"opacity": "0"
			}, 500);
		},
			...
			for (var i = 0; i < 5; i++) {
				(function(i) {
					$("#js-nav").find("span").eq(i).click(function() {
						switch (i) {
							case 0:
								window.open("index.html#_lan=" + _lan, "_self");
								break;
							case 1:
								window.open("profile.html#_lan=" + _lan, "_self");
								break;
							case 2:
		...
		languageClick = function(_div, _i) {
			$(_div).eq(_i).click(function() {
				...
				for (j = 0; j < 3; j++) {
					...
					$("#js-language span").eq(_lan).css({
						"display": "block"
					});
					$("#js-down_lan a").eq(j).css({
						"color": "#000"
					});
					$("#js-down_lan a").eq(_lan).css({
						"color": "#eee"
